...
export class FacebookGroupNavigator {
  ...
  public async navigateToGroup(page: Page, groupUrl: string): Promise<GroupInfo | null> {
    try {
      ...
      scraperLogger.info(`✅ Successfully navigated to group: ${groupInfo.name}`);
      return groupInfo;
    } catch (error) {
  ...
  private async extractGroupInfo(page: Page, groupUrl: string): Promise<GroupInfo> {
    try {
      ...
      for (const selector of restrictedSelectors) {
        try {
          ...
          if (element) {
            groupInfo.canAccess = false;
            if (selector.includes('private') || selector.includes('Join Group')) {
      ...
      try {
        ...
        for (const selector of memberSelectors) {
          try {
            ...
            if (memberElement) {
              const memberText = await page.evaluate(el => el.textContent, memberElement);
      ...
      try {
        ...
        for (const selector of descriptionSelectors) {
          try {
            ...
            if (descElement) {
              const description = await page.evaluate(el => el.textContent?.trim(), descElement);
  ...
  private async attemptToJoinGroup(page: Page): Promise<boolean> {
    try {
      ...
      if (canAccess) {
        ...
        scraperLogger.warn('⚠️ Join attempt unclear, group may require approval');
        return false;
      }
    } catch (error) {
  ...
  private async navigateToPostsFeed(page: Page): Promise<void> {
    try {
      ...
      scraperLogger.warn('⚠️ Could not navigate to posts feed, continuing anyway');
    }
  }

  public async validateGroupUrls(urls: string[]): Promise<{ valid: string[], invalid: string[] }> {
    const valid: string[] = [];
