...
export class ApifyService {
   ...
   */
  async scrapeFacebookGroups(config: ApifyScrapingConfig): Promise<ApifyPost[]> {
    try {
      const inputData = {
      ...
      apiLogger.info('Starting Apify Facebook Groups scraping', { config: inputData });
  ...
  async startScraping(config: ApifyScrapingConfig): Promise<{ runId: string }> {
    try {
      ...
      apiLogger.info('Starting async Apify Facebook Groups scraping', { config: inputData });
      ...
      apiLogger.info('Apify async scraping started', { runId: runData.data.id });
   ...
   */
  async getRunStatus(runId: string): Promise<{ status: string; statusMessage?: string }> {
    try {
      const response = await fetch(
  ...
  async getDatasetItems(datasetId: string): Promise<ApifyPost[]> {
    try {
      ...
      apiLogger.error('Error fetching Apify dataset items', { error, datasetId });
      throw error;
    }
  }
}
