...
export class ExternalJobFilterService {
  private static readonly API_BASE_URL = "https://cmfwyhx1d1rtsjxgteybbbnme.agent.a.smyth.ai";
  private static readonly EXTRACT_ENDPOINT = "/api/extract_job_posts";
   ...
   */
  static async filterAndStructureJobs(jobPostsJson: string): Promise<ExternalJobExtractionResponse> {
    try {
      console.log('🔄 Sending job posts to external API for filtering...');
      
      const response = await fetch(`${this.API_BASE_URL}${this.EXTRACT_ENDPOINT}`, {
        method: 'POST',
        headers: {
      ...
      console.log('✅ Successfully received structured job posts from external API');
   ...
   */
  static async processBatch(posts: unknown[], batchSize: number = 10): Promise<StructuredJobPost[]> {
