...
export class JobPostExtractor {
  ...
  private static isJobPost(post: FacebookPost): boolean {
    ...
    const jobKeywords = [
      'hiring', 'job', 'position', 'vacancy', 'opportunity', 'career',
      'developer', 'engineer', 'programmer', 'designer', 'manager',
      'analyst', 'coordinator', 'specialist', 'consultant', 'intern',
      'remote', 'full-time', 'part-time', 'contract', 'freelance',
      'apply', 'cv', 'resume', 'qualification', 'experience',
      'salary', 'benefit', 'requirement', 'responsibility'
    ];
   ...
   */
  private static extractSingleJobPost(post: FacebookPost): JobPostExtractionResult | null {
  ...
  private static extractVacancies(text: string): number {
    const match = text.match(/(\d+)\s+(?:vacancy|vacancies|position|positions|opening|openings)/i);
  ...
  private static extractCategory(text: string): string {
    const categories = [
      ...
      'devops', 'qa', 'testing', 'ui/ux', 'design', 'marketing',
  ...
  private static extractApplicationMethods(text: string): Array<{type: string, value: string, notes: string}> {
