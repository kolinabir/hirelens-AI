...
export const env: EnvConfig = {
  ...
  facebookPassword: validateEnvVar('FACEBOOK_PASSWORD', process.env.FACEBOOK_PASSWORD),
  
  // MongoDB Configuration
  mongodbUri: validateEnvVar('MONGODB_URI', process.env.MONGODB_URI, 'mongodb://localhost:27017/job-scraper'),
  mongodbDbName: validateEnvVar('MONGODB_DB_NAME', process.env.MONGODB_DB_NAME, 'job_scraper'),
  
  // Application Configuration
  nodeEnv: validateEnvVar('NODE_ENV', process.env.NODE_ENV, 'development'),
  port: parseNumber(process.env.PORT, 3000),
  baseUrl: validateEnvVar('BASE_URL', process.env.BASE_URL, 'http://localhost:3000'),
  ...
  // Security
  jwtSecret: validateEnvVar('JWT_SECRET', process.env.JWT_SECRET, 'default-jwt-secret-change-in-production'),
  encryptionKey: validateEnvVar('ENCRYPTION_KEY', process.env.ENCRYPTION_KEY, 'default-32-char-encryption-key!!'),
  ...
  proxyPort: process.env.PROXY_PORT ? parseNumber(process.env.PROXY_PORT, 0) : undefined,
