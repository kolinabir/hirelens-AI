...
export default function WebsiteJobsList({ 
  ...
  const fetchWebsiteJobs = async () => {
    try {
      ...
      allJobs.sort((a, b) => new Date(b.scrapedAt).getTime() - new Date(a.scrapedAt).getTime());
    ...
    return (
      <div className="bg-red-50 border border-red-200 rounded-lg p-4">
        <div className="flex">
          ...
          <div className="ml-3">
            <h3 className="text-sm font-medium text-red-800">Error loading website jobs</h3>
  ...
  if (jobs.length === 0) {
    return (
      <div className="text-center py-8">
        ...
        <h3 className="mt-2 text-sm font-medium text-gray-900">No website jobs found</h3>
        <p className="mt-1 text-sm text-gray-500">
          {websiteId ? "This website hasn't been scraped yet." : "No websites have been scraped yet."}
        </p>
      </div>
    );
  }
          ...
          >
                ...
                <div className="space-y-1 text-sm text-gray-600 mb-4">
                  <div className="flex items-center">
                    ...
                    {job.positionRole && job.positionRole !== job.jobTitle && (
                      <span className="text-gray-500 ml-2">• {job.positionRole}</span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center">
                    ...
                    <span className="text-blue-600 font-medium">{job.websiteName}</span>
                    <span className="text-gray-400 ml-2">• {new Date(job.scrapedAt).toLocaleDateString()}</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  ...
                  {job.salaryCompensation && (
                    <div>
                      ...
                      <span className="ml-2 text-gray-600">{job.salaryCompensation}</span>
                    </div>
                  )}
                </div>
            ...
            <div className="mt-6 flex items-center justify-between pt-4 border-t border-gray-200">
              <div className="flex items-center space-x-2 text-xs text-gray-500">
                <span>Scraped on {new Date(job.scrapedAt).toLocaleString()}</span>
              </div>
              ...
              <div className="flex items-center space-x-3">
                <button
                  onClick={() => {
                    ...
                    window.open(`/dashboard/jobs?search=${searchQuery}`, '_blank');
                  }}
                  className="inline-flex items-center px-3 py-2 border border-gray-300 shadow-sm text-sm leading-4 font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                >
                  <svg className="-ml-0.5 mr-2 h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
